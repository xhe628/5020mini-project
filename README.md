...
这个任务的核心问题是需要设计一套稳健的方法论，根据火灾发生地点与农田的相对位置以及火灾发生时间与收获季节的相对关系对火灾进行分类。最终输出应包含定量估计（例如：“我们把X%的火灾归类为可能的农业焚烧”）和显示不同火灾类型的地图。

### 2.1数据提取

首先先下载对应不同日期下的作物成熟度的文件（参考见：Heilongjiang_Maize_MA_2010.tif）

然后利用python调用rasterio库提取每个栅格点对应每个位置的坐标的栅格点对应的doy 值并保存进入excle文件
...
此外 corn_raster_paths 需要替换为文件的实际路径
